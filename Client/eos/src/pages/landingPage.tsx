...
const LandingPage = () => {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
        <aside className={`relative bg-white shadow-lg text-white flex flex-col items-center py-6 transition-all duration-300 ${ collapsed ? 'w-20' : 'w-64' }`} >
        {/* Profile Section */}
            <div className={`relative flex ${collapsed ? 'flex-col items-center' : 'flex-row items-center justify-between'} w-full px-4`}>
                {/* Profile Image */}
                <img src={Profile} alt="User" className={`rounded-lg border-2 border-blue-900 transition-all ${ collapsed ? 'w-12 h-12' : 'w-13 h-13' }`} />
                ...
                <button onClick={() => setCollapsed(!collapsed)} className={`text-blue-900 z-10 transition-all ${ collapsed ? 'mt-2' : '' }`} >
                {collapsed ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
                </button>
            </div>

            {/* Full-width Divider Line */}
            <div className="w-3/4 border-b border-blue-900 my-4 mx-2" />


            {/* Navigation */}
            <nav className={`flex flex-col space-y-4 mt-4 w-full ${ collapsed ? 'items-center' : 'px-6' }`} >
                <SidebarButton to="Form" icon={<Home size={20} />} label="Building" active={pathname.includes('/admin/Form')} collapsed={collapsed} />
...
const SidebarButton = ({ to, icon, label, active, collapsed, }: { to: string; icon: JSX.Element; label: string; active: boolean; collapsed: boolean; }) => (
  <Link to={to} className={`flex items-center gap-3 px-4 py-2 rounded-md text-sm font-medium transition text-blue-900 ${ active ? 'bg-[#2c4566] text-white' : 'hover:bg-[#2c4566] hover:text-white' } ${collapsed ? 'justify-center px-2' : ''}`} >
    {icon}
